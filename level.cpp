...
//Parse the xml or *.tmx file
	...
	//Get the enure width and height of the whole map and store it in _size
	int width;
	int height;
	...
	//Get the widt and height of the tiles and store it in _tileSize
	int tileWidth;
	int tileHeight;
	...
	printf("store width and height of map\n");
			...
			if (pData != NULL) {
				while (pData) {
					...
					if (pTile != NULL) {
						int tileCounter = 0;
						while (pTile) {
							//build each individual tile. If gid is 0, no tile is drawn
							if (pTile->IntAttribute("gid") == 0) {
								...
								//get the tileset for the specfic gid
							int gid = pTile->IntAttribute("gid");
							Tileset tls;
							for (int i = 0; i < this->_tilesets.size(); i++) {
								if (this->_tilesets[i].FirstGid <= gid) {
										tls = this->_tilesets.at(i);
										printf("get the tileset for the specific gid\n");
										break;
			...
			//other objects go here
				...
				if (pObject != NULL) {
					while (pObject) {
						...
						XMLElement* pPolyline = pObject->FirstChildElement
						("polyline");
						if (pPolyline != NULL) {
							std::vector<std::string> pairs;
							const char* pointString = pPolyline->Attribute
							("points");
							std::stringstream ss;
							ss << pointString;
							Utils::split(ss.str(), pairs, ' ');
							//Loop throught the list of pairs and slpit them in to Vector2's and store them in th
							//points vector.
							for (int i = 0; i < pairs.size(); i++) {
								std::vector<std::string> ps;
								Utils::split(pairs.at(i), ps, ',');
								points.push_back(Vector2(std::stoi(ps.at(0)),
									std::stoi(ps.at(1))));
							}
						}
					...
					while (pObject) {
						...
						if (ss.str() == "player") {
							this->_spawnPoint = Vector2(std::ceil(x) *
								globals::SPRITE_SCALE, std::ceil(y) * globals::SPRITE_SCALE);
						}
						pObject = pObject->NextSiblingElement("object");
					}
				}
